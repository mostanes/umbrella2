...
namespace Umbrella2.Algorithms.Images
{
	public class SegmentDetector
	{
					...
					if (true)
					{
						...
						while (TagsMod)
						{
							...
							for (int k = 0; k < IntermediateList.Count; k++)
								...
								{
									double dangle = Math.Abs(IntermediateList[k].EigenAngle1 - IntermediateList[l].EigenAngle1);
		...
		List<MedianDetection> GetMedetect(FitsImage InputImage, ObservationTime ObservationTime)
		{
			...
			var x1 = DetectedPP.Zip(DetectedPV, (x, y) => new MedianDetection(InputImage.Transform, ObservationTime, x, y));
