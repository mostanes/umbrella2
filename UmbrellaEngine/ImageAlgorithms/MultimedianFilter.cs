...
{
	public static class MultiMedianFilter
	{
		/// <summary>
		/// Runs a semi-median filtering using a weighted method and geometric averaging on a single image.
		/// </summary>
		/// <param name="Input">Input image.</param>
		/// <param name="Output">Output image.</param>
		/// <param name="PSF">PSF weights.</param>
		public static void RunMedian(FitsImage Input, FitsImage Output, double[,] PSF, ImageStatistics imStat)
		{
			...
			Parallel.For(0, Input.Height / ThreadStep, (x) => SingleImageBlock(Input, Output, imStat, PSFLinear, (PSF.GetLength(0) / 2), (int) x * ThreadStep, LineStep, (int) (x + 1) * ThreadStep));
			if (Input.Height % ThreadStep != 0) SingleImageBlock(Input, Output, imStat, PSFLinear, (PSF.GetLength(0) / 2), (int) (Input.Height - Input.Height % ThreadStep), LineStep, (int) Input.Height);
			//SingleImageBlock(Input, Output, PSFLinear, (PSF.GetLength(0) - 1) / 2, 0, 50, (int) Input.Height);
		}

		static void RunSingleMedian(double[,] Input, double[,] Output, double[] PSF, int Size, ImageStatistics imStat)
		{
		...
		static void SingleImageBlock(FitsImage Input, FitsImage Output, ImageStatistics imStat, double[] PSFLinearized, int Size, int StartLine, int LineStep, int LEnd)
		{
			ImageData InputData;
			...
			for (CLine = StartLine; CLine < LEnd; CLine += LineStep)
			{
				...
				RunSingleMedian(InputData.Data, OutputData.Data, PSFLinearized, Size * 2 + 1, imStat);
			}
